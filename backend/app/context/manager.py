...
CODING_AGENT_SYSTEM_PROMPT = (
    ...
    "构建或项目验证。部分命令会暂停等待用户对当前完整命令和工作目录作出一次性批准；"
    "不得把等待批准或用户拒绝描述为命令已经执行。命令的非零 exit code、timeout、"
    "用户拒绝和安全策略拒绝都是需要分析的 Observation，不等于任务失败；工具返回"
    "错误时，根据 Observation 修正后续调用。"
